        ...
        <div class="modal-body">
          ...
          <div class="search-results">
            <div class="results-header">
              <span>検索結果: {{ filteredCustomers.length }}件</span>
            </div>
                ...
                <tbody>
                  <tr v-for="customer in filteredCustomers" :key="customer.id">
                    ...
                    <td class="text-center">
                      <button @click="selectCustomer(customer)" class="btn-select">選択</button>
                    </td>
                  </tr>
                  <tr v-if="filteredCustomers.length === 0">
                    <td colspan="3" class="no-data">該当する得意先がありません</td>
                  </tr>
                </tbody>
              </table>
            </div>
...
const filterCustomers = () => {
  filteredCustomers.value = props.customers.filter(customer => {
    ...
    if (searchConditions.value.code) {
      if (!customer.code.toLowerCase().includes(searchConditions.value.code.toLowerCase())) {
        return false
      }
    }
